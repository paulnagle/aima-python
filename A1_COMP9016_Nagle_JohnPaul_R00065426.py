...
def building_your_world(obstacle_pos, positive_pos, negative_pos, agent_pos):
    ...
    agent_list = [TableDrivenAgent().table_action]
    print("\nAGENT RESULTS")
    for agent_program in agent_list:
        ...
        print(f"{agent_program.__name__:20}:  Cost: {avg_performance:.2f} Win Rate: {win_rate:.2f}% ({agent_stats['wins']}/{args.runs})")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-s', '--steps', type=int, required=True, help='Number of steps per run to attempt to win the game (mandatory)')
    parser.add_argument('-r', '--runs', type=int, required=True, help='Number of times to run each agent (mandatory)')
    parser.add_argument('-w', '--width', type=int, required=True, help='Width of the grid world (mandatory)')
    parser.add_argument('-d', '--depth', type=int, required=True, help='depth of the grid world (mandatory)')
